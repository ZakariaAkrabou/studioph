...
import { useLoginAdminMutation } from '../../../store/services/authApi.jsx';
...
const Login = () => {
  ...
  const [login, { isLoading: isAuthLoading }] = useLoginAdminMutation();
    ...
    <div className="min-h-screen grid grid-cols-1 md:grid-cols-2 bg-gray-950">
      ...
      <div className="relative hidden md:flex flex-col p-10 overflow-hidden text-white">
        ...
        <div className="relative z-10 flex-1 flex flex-col justify-center">
          ...
          <div className="mt-10">
            <h2 className="text-4xl font-bold leading-tight">Capture. Curate. Create.</h2>
            <p className="mt-3 text-white/70 max-w-md">Streamline your photography categories and portfolios with a clean, focused dashboard experience.</p>

            <div className="mt-6 flex flex-wrap gap-2 max-w-sm">
              {['Weddings', 'Portraits', 'Events', 'Fashion', 'Lifestyle', 'Travel'].map((category) => (
                <span key={category} className="px-3 py-1 text-xs rounded-full bg-white/10 border border-white/10 backdrop-blur">
                  {category}
                </span>
              ))}
            </div>
          ...
          <div className="bg-white/80 backdrop-blur-xl border border-white/50 shadow-xl rounded-2xl p-8">
            <div className="flex items-center gap-3 justify-center">
              ...
              <div>
                ...
                <p className="text-sm text-gray-500">Sign in to manage categories and portfolio</p>
              </div>
            </div>
            ...
            <form className="mt-8 space-y-5" onSubmit={handleSubmit}>
                ...
                <div>
                  ...
                  <div className="relative">
                    ...
                    <input
                      ...
                      aria-invalid={touched.email && !/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(formData.email)}
              ...
              <div className="flex items-center justify-between">
                ...
                <Link to="/auth/forgot-password" className="text-sm font-medium text-indigo-600 hover:text-indigo-500">Forgot password?</Link>
              </div>

              <button
                type="submit"
                disabled={isAuthLoading}
                className={`w-full inline-flex items-center justify-center rounded-lg px-4 py-2.5 text-sm font-medium text-white bg-indigo-600 hover:bg-indigo-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500 shadow-md ${isAuthLoading ? 'opacity-75 cursor-not-allowed' : ''}`}
              >
                {isAuthLoading ? (
                  <>
              ...
              <div className="text-center text-sm text-gray-600">
                ...
                <Link to="/auth/register" className="font-medium text-indigo-600 hover:text-indigo-500">Create one</Link>
              </div>
            </form>
          </div>
        </div>
